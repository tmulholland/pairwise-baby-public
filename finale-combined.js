...
async function loadCombinedRanking() {
  ...
  try {
    ...
    const payload = await apiFetchJson(`/api/finale/combined${query ? `?${query}` : ''}`);
    applyState(payload);
    setStatus('Live');
  } catch (error) {
    setStatus('Error');
...
function renderRanking() {
  ...
  if (!state.combinedRanking.names.length) {
    renderEmpty('No combinations yet', 'Add first and middle names to begin.');
    return;
  }
...
function initializePage() {
  ...
  elements.backToComparingLink.href = activeUser ? `/finale/${activeUser}` : '/finale';
}
