...
async function loadResults() {
  ...
  try {
    const payload = await apiFetchJson('/api/showtime/results');
...
async function apiFetchJson(url, options) {
  ...
  try {
    ...
    throw new Error('The server returned a non-JSON response. Refresh and try again.');
  }
...
function renderRankings(rankings) {
  ...
  for (const ranking of rankings) {
    ...
    title.textContent = 'Current shortlist';
...
function initializePage() {
  ...
  elements.backToComparingLink.href = activeUser ? `/showtime/${activeUser}` : '/showtime';
}

function getLastActiveUser() {
  return normalizeSlug(window.localStorage.getItem(LAST_ACTIVE_USER_KEY));
}
