const USER_CONFIRM_PREFIX = 'showtime-confirmed-user:';
const LAST_ACTIVE_USER_KEY = 'showtime-last-active-user';
const PRIORITIZE_FEWER_KEY = 'showtime-prioritize-fewer-comparisons';
...
const elements = {
  ...
  sourceAddForm: document.querySelector('#source-add-form'),
  sourceNameSelect: document.querySelector('#source-name-select'),
  sourceAddButton: document.querySelector('#source-add-button'),
...
function bindEvents() {
  ...
  elements.sourceAddForm.addEventListener('submit', handleSourceAdd);
...
async function loadState() {
  ...
  try {
    const payload = await apiFetchJson(`/api/showtime/state/${encodeURIComponent(state.activeUser)}`);
    applyState(payload);
  } catch (error) {
...
async function handleSourceAdd(event) {
  event.preventDefault();
  const selectedName = elements.sourceNameSelect.value;

  if (!selectedName) {
    return;
  }

  await addNames([selectedName]);
}

async function addNames(names) {
  try {
    const payload = await apiFetchJson('/api/showtime/names', {
      method: 'POST',
...
async function deleteName(nameId, name) {
  const confirmed = window.confirm(`Delete ${name} from the showtime shortlist? This removes it from every showtime user's final rankings and comparison history.`);
  ...
  try {
    const payload = await apiFetchJson(`/api/showtime/names/${nameId}?userSlug=${encodeURIComponent(state.activeUser)}`, {
      method: 'DELETE',
    });
    applyState(payload.state);
  } catch (error) {
...
async function submitComparison(winnerId) {
  ...
  try {
    const payload = await apiFetchJson('/api/showtime/comparisons', {
      method: 'POST',
      body: JSON.stringify({
...
function applyState(payload) {
  ...
  state.sourceNames = payload.sourceNames || [];
  currentPair = null;
  render();
}
...
function renderHeader() {
  ...
  elements.currentUserPill.textContent = 'Isolated shortlist';
...
function renderSourceNames() {
  elements.sourceNameSelect.innerHTML = '';

  const availableNames = state.sourceNames.filter((entry) => !entry.included);
  const placeholderOption = document.createElement('option');
  placeholderOption.value = '';
  placeholderOption.textContent = availableNames.length ? 'Choose a main-pool name' : 'All main-pool names already imported';
  elements.sourceNameSelect.append(placeholderOption);

  for (const entry of availableNames) {
    const option = document.createElement('option');
    option.value = entry.name;
    option.textContent = formatSourceNameLabel(entry);
    elements.sourceNameSelect.append(option);
  }

  elements.sourceNameSelect.disabled = !availableNames.length;
  elements.sourceAddButton.disabled = !availableNames.length;
}

function formatSourceNameLabel(entry) {
  const average = Math.round(entry.averageRating);
  const rank = entry.ssaRank ? `, SSA #${entry.ssaRank}` : '';
  return `${entry.name} (${average} avg${rank})`;
}
...
function renderManualCompareControls() {
  ...
  if (state.names.length < 2) {
    ...
    elements.manualCompareNote.textContent = 'Add at least two finalist names to compare manually.';
    return;
  }
...
function renderMatchup() {
  ...
  if (!enoughNames) {
    elements.matchupState.innerHTML = '<p>Add at least two finalist names to start comparing.</p>';
...
function renderPersonalRanking() {
  ...
  if (!activeRanking) {
    ...
    emptyScore.textContent = 'Compare finalists to build your order.';
  ...
  if (!activeRanking.names.length) {
    ...
    emptyName.className = 'ranking-name';
    emptyScore.className = 'ranking-score';
    emptyName.textContent = 'No finalists yet';
    emptyScore.textContent = 'Import or add names to begin.';
    empty.append(emptyName, emptyScore);
    elements.personalRankingList.append(empty);
    return;
  }
...
function getChoiceSummary(entry) {
  ...
  return 'No quick origin note found for this finalist yet.';
}
...
function getUserSlugFromPath() {
  const slug = normalizeSlug(window.location.pathname.replace(/^\/showtime\/+/, ''));
...
function setLoading(isLoading) {
  elements.currentUserPill.textContent = isLoading ? 'Loading' : 'Isolated shortlist';
}
